$(function(){
	...
	//		console.log(info.view.type);
			...
			var dataDateString = "";
			if($(info.el).find(".fc-time").length > 0){ // ถ้ามีการแสดงเวลา
				if(info.event.end !== null){ // มีเวลาสิ้นสุด
					...
					$(info.el).find(".fc-time").text(dataDateString);
				}else{
		...
		viewSkeletonRender: function(info) {
			if(info.view.type.match(/time/)){
				$("div.fc-slats").find("tr").each(function(i, el){
					var dataTime = $(el).data("time");
					$(el).find("span").text(dataTime.substring(0,5));
				});
			}
		},
